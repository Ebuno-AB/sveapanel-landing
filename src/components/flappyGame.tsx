...
export default function FlappyBirdCanvas() {
  ...
  const stateRef = useRef<{
    ...
    birdVY: number;
    angle: number;
    pipes: { x: number; top: number; gap: number; passed: boolean; id: number }[];
    score: number;
    best: number;
    money: number;
    groundX: number;
  ...
  const loop = (now: number) => {
    ...
    ctx.translate(-bx, -by);
    drawBird(ctx, bx, by, s.mode === 'celebrating' || s.mode === 'dead' ? 'celebrate' : 'fly', s.flyFrame);
    ctx.restore();
  ...
  const drawPipe = (ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) => {
    // body
  ...
  const drawCenterText = (ctx: CanvasRenderingContext2D, text: string, size: number) => {
  ...
  const drawSubText = (ctx: CanvasRenderingContext2D, text: string, size: number, offset = 0, color = '#fff') => {
  ...
  const drawBoard = (ctx: CanvasRenderingContext2D, score: number, best: number, money: number) => {
    const w = 260;
    const h = 160;
