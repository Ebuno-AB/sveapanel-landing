...
import { publicRoutes, protectedRoutes, setupRoutes } from "./routes/routes";
...
function AppRoutes() {
  ...
  return (
    <Routes>
      {/* Setup routes (requires auth, only for incomplete setup) */}
      {setupRoutes.map((route, i) => (
        <Route key={`setup-${i}`} path={route.path} element={route.element}>
